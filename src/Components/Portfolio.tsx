import { Box, Table, TableCaption, Tbody, Td, Th, Thead, Tr, Button, Tooltip } from '@chakra-ui/react';
...
}> = ({ portfolio, onRemoveFromPortfolio }) => {
  return (
      ...
      <Tbody>
        {portfolio.map((item) => {
          return (
            <Tr key={nanoid()}>
              <Td fontSize={['sm', 'md']}>
                <NavLink to={`/details/${item['1. symbol']}`}><Tooltip label="Click here for details!">{`⭐ ${item['2. name']}`}</Tooltip></NavLink>
              </Td>
